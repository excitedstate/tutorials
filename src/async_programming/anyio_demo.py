...
async def anyio_sync_primitives():
    ...
    # 并发执行多个递增操作
    await anyio.gather(*[increment_with_lock() for _ in range(10)])
    ...
    # 并发执行5个任务，但受信号量限制
    await anyio.gather(*[task_with_semaphore(f"Task-{i}") for i in range(5)])
    ...
    # 同时运行两个任务
    await anyio.gather(wait_for_event(), trigger_event())
...
async def anyio_stream_demo():
    ...
    async def echo_client():
        ...
        try:
            ...
            async with await anyio.connect_tcp("127.0.0.1", 12345) as client_sock:
                ...
                await client_sock.sendall(message.encode())
    ...
    # 启动服务器和客户端
    try:
        async with await anyio.create_tcp_listener(local_host="127.0.0.1", local_port=12345) as server_sock:
            # 同时运行服务器和客户端
            result = await anyio.gather(echo_server(server_sock), echo_client(), return_exceptions=True)
            print(f"TCP示例结果: {result[1] if len(result) > 1 else False}")
    except Exception as e:
        print(f"TCP示例失败: {e}")
