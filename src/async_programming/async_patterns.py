...
import asyncio
import time
from typing import AsyncGenerator, List, Any, AsyncIterator
...
async def async_event_loop_demo():
    ...
    print(f"任务1状态: {task1_obj.state}")
    print(f"任务2状态: {task2_obj.state}")
    
    # 等待任务完成
    print("\n等待任务完成...")
    results = await asyncio.gather(task1_obj, task2_obj)
    
    print(f"\n任务1状态: {task1_obj.state}")
    print(f"任务2状态: {task2_obj.state}")
...
async def async_context_manager():
    ...
    async def use_async_resource():
        ...
        # 嵌套使用异步上下文管理器
        async with AsyncResource("Resource-2") as resource2, \
