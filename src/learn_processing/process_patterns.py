...
def producer_consumer_with_queue():
    ...
    # 创建一个队列，用于进程间通信
    queue = multiprocessing.Queue[str](maxsize=5)
...
def pipe_communication_demo():
    ...
    sender_process = multiprocessing.Process(
        target=sender, args=(child_conn,), name="Sender"
    )
    receiver_process = multiprocessing.Process(
        target=receiver, args=(parent_conn,), name="Receiver"
    )
...
def shared_memory_demo():
    ...
    update_process = multiprocessing.Process(
        target=update_shared_memory, name="Update-Process"
    )
...
def reader_writer_pattern():
    ...
    for i in range(3):
        reader_process = multiprocessing.Process(
            target=reader, args=(i,), name=f"Reader-{i}"
        )
