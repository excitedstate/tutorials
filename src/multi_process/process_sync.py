...
def process_semaphore_demo():
    ...
    processes = []
    for i in range(5):
        p = multiprocessing.Process(target=access_resource, args=(i,), name=f"Resource-User-{i}")
        processes.append(p)
        p.start()
...
def process_event_demo():
    ...
    processes = []
    for i in range(3):
        p = multiprocessing.Process(target=wait_for_event, args=(i,), name=f"Event-Waiter-{i}")
        processes.append(p)
        p.start()
...
def process_barrier_demo():
    ...
    processes = []
    for i in range(3):
        p = multiprocessing.Process(target=worker, args=(i,), name=f"Barrier-Worker-{i}")
        processes.append(p)
        p.start()
