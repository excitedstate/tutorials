...
async def async_concurrency_with_gather():
    ...
    results = await asyncio.gather(
        task("A", 2), task("B", 1), task("C", 3), task("D", 1.5)
    )
...
async def async_concurrency_with_as_completed():
    ...
    # 创建任务列表
    task_coros = [task("A", 2), task("B", 1), task("C", 3), task("D", 1.5)]
...
async def async_limits_with_semaphore():
    ...
    async def limited_task(
        semaphore: asyncio.Semaphore, name: str, delay: float
    ) -> str:
        """受信号量限制的异步任务"""
    ...
    # 创建多个任务
    task_coros = [limited_task(semaphore, f"Task-{i}", 1) for i in range(5)]
...
async def async_cancellation_demo():
    ...
    async def cancellable_task(name: str, delay: int) -> str:
        """可取消的异步任务"""
        try:
